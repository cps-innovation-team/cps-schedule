...
func pickerFormatter(color: UIColor) -> Int {
...
func mainPickerFormatter(color: UIColor) -> Int {
    return(1)
}
        ...
        VStack {
            ...
            Form {
                Section {
                    ...
                    if (title.contains("Block") || title.contains("Advising") || title.contains("Compass")) {
                    ...
                    if pickervalue == 0 {
                        HStack {
                            ...
                            .onReceive([self.colorvalue].publisher.first(), perform: { value in
                                UserDefaults(suiteName: "group.TheCollegePreparatorySchool.ScheduleApp")?.setColor(color: UIColor(receiveColor(value: self.colorvalue)), forKey: colorkey)
                                WidgetCenter.shared.reloadAllTimelines()
                            })
                            .labelsHidden()
                            ...
                            VStack {
                                ColorPicker("Set a custom color", selection: $color, supportsOpacity: false)
                                    .onChange(of: color, perform: { value in
                                        UserDefaults(suiteName: "group.TheCollegePreparatorySchool.ScheduleApp")?.setColor(color: UIColor(color), forKey: colorkey)
                                        WidgetCenter.shared.reloadAllTimelines()
                                    })
                            }
                            Spacer()
