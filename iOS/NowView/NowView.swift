                    ...
                    ForEach(array, id: \.id) { entry in
                        if entry.title.t.contains("C") && entry.content.t == convertDatetoString(input: date) {
                            Button(action: {
                                ...
                                .background(RoundedRectangle(cornerRadius: 11).foregroundColor(Color(.quaternarySystemFill)))
...
//                UIPasteboard.general.string = "https://www.college-prep.org/2020-21-school-year"
            }, label: {
                ZStack {
                    RoundedRectangle(cornerRadius: 12.5)
                        .foregroundColor(colorScheme == .light ? Color(UIColor(Color("QLGreen")).tinted(amount: 0.25)) : Color(UIColor(Color("QLGreen")).shaded(amount: 0.25)))
                        .frame(height: frameheight)
                    BookmarksCell(name: "COVID-19 Info", image: "info.circle")
                }
            }).buttonStyle(ScaleButtonStyle())
            Button (action: {
                openURL(URL(string: "https://docs.google.com/document/d/1JOSWem1nFJZRoVMWGDN8FYBcMftzHRi-5yZ283NTBpE/edit")!)
//                UIPasteboard.general.string = "https://docs.google.com/document/d/1JOSWem1nFJZRoVMWGDN8FYBcMftzHRi-5yZ283NTBpE/edit"
            }, label: {
                ZStack {
                    RoundedRectangle(cornerRadius: 12.5)
                        .foregroundColor(colorScheme == .light ? Color(UIColor(Color("QLGreen")).tinted(amount: 0.25)) : Color(UIColor(Color("QLGreen")).shaded(amount: 0.25)))
...
//                UIPasteboard.general.string = "https://college-prep.myschoolapp.com/app/student#studentmyday/progress"
            }, label: {
                ZStack {
                    RoundedRectangle(cornerRadius: 12.5)
                        .foregroundColor(colorScheme == .light ? Color(UIColor(Color("AccentColor")).tinted(amount: 0.25)) : Color(UIColor(Color("AccentColor")).shaded(amount: 0.25)))
                        .frame(height: frameheight)
...
//                UIPasteboard.general.string = "https://cpsradar.com"
            }, label: {
                ZStack {
                    RoundedRectangle(cornerRadius: 12.5)
                        .foregroundColor(colorScheme == .light ? Color(UIColor(Color("AccentColor")).tinted(amount: 0.25)) : Color(UIColor(Color("AccentColor")).shaded(amount: 0.25)))
...
//                UIPasteboard.general.string = "https://sites.google.com/thecollegepreparatoryschool.org/digitallc/home"
            }, label: {
                ZStack {
                    RoundedRectangle(cornerRadius: 12.5)
                        .foregroundColor(colorScheme == .light ? Color(UIColor(Color(.systemPink)).tinted(amount: 0.25)) : Color(UIColor(Color(.systemPink)).shaded(amount: 0.25)))
                        .frame(height: frameheight)
...
//                UIPasteboard.general.string = "https://collegepreplibrary.com"
            }, label: {
                ZStack {
                    RoundedRectangle(cornerRadius: 12.5)
                        .foregroundColor(colorScheme == .light ? Color(UIColor(Color(.systemPink)).tinted(amount: 0.25)) : Color(UIColor(Color(.systemPink)).shaded(amount: 0.25)))
...
//            UIPasteboard.general.string = "https://docs.google.com/forms/d/e/1FAIpQLSdeFnyD4Ti1A5pKaLisd81IeX0WBtQ6aBZFa47CeNaw3trS3Q/viewform?usp=sf_link"
        }, label: {
            ZStack {
                RoundedRectangle(cornerRadius: 12.5)
                    .foregroundColor(colorScheme == .light ? Color(UIColor(Color(.systemPurple)).tinted(amount: 0.25)) : Color(UIColor(Color(.systemPurple)).shaded(amount: 0.25)))
                    .frame(height: frameheight)
        ...
        Group {
            if (vSizeClass == .regular && hSizeClass == .regular) {
                ScrollView(.vertical, showsIndicators: false, content: {
                    VStack(spacing: 20) {
                        ...
                        if authstatus {
                            ...
                            NavigationLink(destination: ClubsView(date: $currentDate, array: $clubsArray), tag: "clubs", selection: $selection) {
                                ClubsQuickLink(date: $currentDate, array: $clubsArray)
                            }
                            .buttonStyle(ScaleButtonStyle())
                            NavigationLink(destination: SportsView(date: $currentDate, array: $sportsArray), tag: "sports", selection: $selection) {
                ...
                NavigationView {
                    ...
                    .navigationTitle("Dashboard")
                    .onOpenURL { url in
                        if case .clubs = url.detailPage {
                            selection = "clubs"
                        }
                        if case .sports = url.detailPage {
                            selection = "sports"
                        }
                    }
                }
...
enum PageIdentifier: Hashable {
    case clubs, sports, schedule, dashboard
}
